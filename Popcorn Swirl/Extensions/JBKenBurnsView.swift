...
class JBKenBurnsView: UIView {
    ...
    private let enlargeRatio: CGFloat = 1
    ...
    let screenOrientationAwareness = false
    ...
    @objc internal func deviceOrientationDidChange() {
        ...
        if (newOrientation.isPortrait && !portrait) {
            portrait = true
            didActuallyChange = true
        } else if (newOrientation.isLandscape && portrait) {
            portrait = false
            didActuallyChange = true
        }
